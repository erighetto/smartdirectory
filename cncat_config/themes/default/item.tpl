            ...
            ext_text1, ext_text2, ext_text3, item_title_translite]    
                        ...
                        <tr><td rowspan="4" class="number">
                            ...
                            {IF $CNCAT[config][use_ext_links]}
                                <a {IF $CNCAT[config][link_new_window]}target="_blank"{ENDIF} href="{cn_str($CNCAT[item][_ext_url])}"><strong>{cn_str($CNCAT[item][item_title])}</strong></a>
                            {ELSE}
                                {IF $CNCAT[item][link_target] == 1}
                                    <a {IF $CNCAT[config][link_new_window]}target="_blank"{ENDIF} href="{$CNCAT[item][link_url]}"><strong>{cn_str($CNCAT[item][item_title])}</strong></a>
                                {ENDIF}
                                {IF $CNCAT[item][link_target] == 2}
                                    <a {IF $CNCAT[config][link_new_window]}target="_blank"{ENDIF} href="{$CNCAT[abs]}{$CNCAT[system][dir_prefix]}jump.php?{$CNCAT[item][item_id]}"><strong>{cn_str($CNCAT[item][item_title])}</strong></a>
                                {ENDIF}
