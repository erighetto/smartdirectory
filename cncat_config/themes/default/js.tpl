...
{
        var now = new Date();
        var cb = document.getElementById( "use_" + input );
	var el=document.getElementById(input);
	if ( cb != null )
        {
            cb.checked = true;
        }
	y= el.value ? parseInt(el.value.substring(6,10)) : now.getFullYear();
	m= el.value ? el.value.substring(3,5) : (now.getMonth() + 1);
...
function ShowCalendar(y,m,div, input, nohide) 
{
	...
	var el=document.getElementById(input);
	d= el.value ? el.value.substring(0,2) : now.getDate();
    ...
    d+='<iframe scrolling="no" frameborder="0" src="javascript:void(0);" style="position: absolute; z-index: -1; top: 0; left: 0; width: 100%; height: 100%; filter:alpha(opacity=0)"></iframe>';
