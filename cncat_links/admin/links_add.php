...
if ($_mode == "forms" && isset($_POST["doPost"])) {
    ...
    for ($i = 1; $i <= $_count; $i++) {
        ...
        if (empty($_item[$i]["item_title"])) {
            ...
            $_errors[$i][] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["site_title"], 255), $CNCAT["lang"]["field_to_long"]);
        }
        ...
        if (empty($_item[$i]["link_url"])) {
            ...
            $_errors[$i][] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["link_url"], 255), $CNCAT["lang"]["field_to_long"]);
        }
        ...
        if (cn_strlen($_item[$i]["item_descr"]) > 65000) {
            $_errors[$i][] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["descr"], 65000), $CNCAT["lang"]["field_to_long"]);
        }
        ...
        if (cn_strlen($_item[$i]["item_descr_full"]) > 65000) {
            $_errors[$i][] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["descr_full"], 65000), $CNCAT["lang"]["field_to_long"]);
        }
    ...
    if (!$_errors) {
        for ($i = 1; $i <= $_count; $i++) {
            ...
            if ($_is_edit) {
                ...
                itemUpdateExt(0, $_item_ext[$i], "`item_id`=" . $_item_id);
            } else {
                $_item[$i]["item_image"] = "";
            ...
            foreach ($_cats[$i] as $cat_id) {
                $values[] = "(" . (int)$_item_id . ", " . (int)$cat_id . ", 0, 0, 0)";
            }
        ...
        if ($_is_edit) {
            header("Location: " . (!empty($_POST["referer"]) ? $_POST["referer"] : "index.php?act=links_add"));
        } else {
            header("Location: index.php?act=links_add");
        }
...
?>
<h1><?php print $CNCAT["lang"]["links"]?> / <?php print isset($_GET['edit']) ? $CNCAT["lang"]["editing"] : $CNCAT["lang"]["addition"]?></h1>
<?php
if ($_mode == "forms") {
    $_item_id = (int)$_GET["id"];
    $_is_edit = isset($_GET["edit"]);
...
<table class="form">
<form action="index.php?act=links_add&mode=forms<?php print isset($_REQUEST["edit"]) ? "&edit=1&id=" . $_REQUEST["id"] : ""?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="count" value="<?php print $_count?>" />
    <input type="hidden" name="referer" value="<?php print $_POST["referer"]?>" />
<?php
    for ($link_num = 1; $link_num <= $_count; $link_num++) {
?>
    <tr><td class="title" colspan="2"><?php print $CNCAT["lang"]["link"]?> <?php print $link_num?></td></tr>
    <tr>
        <td class="name"><?php print $CNCAT["lang"]["site_title"]?></td>
        <td class="field"><input type="text" name="item_title[<?php print $link_num?>]" value="<?php print htmlspecialchars($_POST['item_title'][$link_num])?>" class="text" /></td>
    </tr>
    <tr><td class="deline" colspan="2"></td></tr>
    <tr>
        <td class="name"><?php print $CNCAT["lang"]["link_url"]?></td>
...
?>
    ...
    <tr><td class="deline" colspan="2"></td></tr>
    <tr><td colspan="2" class="name"><?php print $CNCAT["lang"]["descr"]?></td></tr>
...
?>
    ...
    <tr><td colspan="2" class="name"><?php print $CNCAT["lang"]["descr_full"]?></td></tr>
    <tr><td colspan="2" class="name">
<?php
...
?>
    <textarea name="item_descr_full[<?php print $link_num?>]" style="width: 100%;"><?php print htmlspecialchars($_POST['item_descr_full'][$link_num])?></textarea>
<?php
}
?>
    </td></tr>
    ...
    <?php print renderExtfields($link_num, (isset($_POST["item_id"][$link_num]) ? $_POST["item_id"][$link_num] : 0))?>
    <tr>
    ...
    <tr>
        <td class="name"><?php print $CNCAT["lang"]["display_ext_type"]?></td>
        <td class="field">
            <select name="item_display_ext[<?php print $link_num?>]">
                <option value="0" <?php print $_POST["item_display_ext"][$link_num] == 0 ? "selected=\"selected\"" : "";?> ><?php print $CNCAT["lang"]["deafult_as_in_settings"]?></option>
                <option value="1" <?php print $_POST["item_display_ext"][$link_num] == 1 ? "selected=\"selected\"" : "";?> ><?php print $CNCAT["lang"]["display"]?></option>
                <option value="2" <?php print $_POST["item_display_ext"][$link_num] == 2 ? "selected=\"selected\"" : "";?> ><?php print $CNCAT["lang"]["not_display"]?></option>
            </select>
        </td>
    </tr>
    <tr>
        <td class="name"><?php print $CNCAT["lang"]["item_favour_level"]?></td>
        <td class="field">
...
<form action="index.php?act=links_add&mode=forms" method="post">
    <tr>
        <td class="name"><?php print $CNCAT["lang"]["links_count"]?></td>
