...
if ($CNCAT["config"]["add_use_captcha"]=="default" || 
    $CNCAT["config"]["add_use_captcha"] == "recaptcha" ||
    $CNCAT["config"]["add_use_captcha"] == "keycaptcha")
{
    require_once $CNCAT["system"]["dir_root"] . $CNCAT["system"]["dir_admin"] . "captcha/{$CNCAT["config"]["add_use_captcha"]}.php";
}
elseif ($CNCAT["config"]["add_use_captcha"])
{
    $CNCAT["config"]["add_use_captcha"] = "default";
    require_once $CNCAT["system"]["dir_root"] . $CNCAT["system"]["dir_admin"] . "captcha/{$CNCAT["config"]["add_use_captcha"]}.php";
}  

$CNCAT_ENGINE->tpl->loadTemplates($CNCAT["config"]["default_theme"], "add");
$CNCAT_ENGINE->loadBanners("add");

$CNCAT['page']['title'] = $CNCAT['lang']['menu_add_item'] . ' / ' . $CNCAT['page']['title'];
...
function cn_render() {   
    ...
    if (!$CNCAT["config"]["add_disable"]) {
        ...
        $result = cn_str_replace("{DISPLAY FORM}", "<tr><td class=\"text\">" . $CNCAT["lang"]["admin_disable_add"] . "</td></tr>", $result);
    }
...
function renderForm() {
    ...
    // if isset next and prev command - reset form
    if (!isset($_GET["submit"]) && !isset($_POST["next"]) || 
        isset($_POST["next"]) && isset($_POST["prev"])) {
        resetForm();
    }

    // set wizard mode
    $wizard = $CNCAT["config"]["add_mode"];
    ...
    if ($step == 2 || $step == 3) {
        ...
        // select item data
        $query = "SELECT `item_title`, `link_url`, `link_back_link_url` FROM `" . $dbPrefix . "items`";
        $query .= " WHERE `item_status`=4 AND `item_id`=" . $item_id;
        $res = $CNCAT_ENGINE->db->query($query, "Item data") or $CNCAT_ENGINE->displayErrorDB(mysql_error());
        ...
        $CNCAT["add"]["readonly_link_url"] = false;
        $CNCAT["add"]["readonly_back_link"] = false;
        ...
        if ($step > 1) {
            $CNCAT["add"]["readonly_link_url"] = true;
        }

        if ($step > 2) {
            $CNCAT["add"]["readonly_back_link"] = true;
        }

        $CNCAT["add"]["link_url"] = $item["link_url"];
        $CNCAT["add"]["back_link"] = $item["link_back_link_url"];
        $CNCAT["add"]["site_title"] = $item["item_title"];
    }
    ...
    if (isset($_POST["next"])) {
        ...
        if (!$wizard || $step == 1) {
            ...
            if (empty($link_url) || (cn_strlen($link_url) < 11)) {
                $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["link_url"], $CNCAT["lang"]["field_empty"]);
            } elseif (strlen($link_url) > $CNCAT["config"]["add_max_url"]) {
                $errors[] = cn_str_replace(
                    array( "%FIELD%", 
                           "%LEN%", 
                           "%COUNT%"), 
                    array( $CNCAT["lang"]["link_url"], 
                           $CNCAT["config"]["add_max_url"], 
                           cn_strlen($link_url)), 
                    $CNCAT["lang"]["field_to_long"]
                );
            } elseif ($CNCAT["config"]["add_check_exists_url"] && checkExistingUrl($link_url)) {
        ...
        if (!$wizard || $step == 2) {
            ...
            if ($CNCAT["config"]["add_use_back_link"]) {
                ...
                $link_url  = $CNCAT["add"]["link_url"];

                if (!empty($back_link) && !preg_match("#^http[s]?://#i", $back_link)) {
                    $back_link = "http://" . $back_link;
                }

                if ((empty($back_link) || (cn_strlen($back_link) < 11)) && $CNCAT["config"]["add_accept_to_add"] == 2) {
                    $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["back_link_url"], $CNCAT["lang"]["field_empty"]);
                } elseif (cn_strlen($back_link) > $CNCAT["config"]["add_max_backurl"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["back_link"], $CNCAT["config"]["add_max_backurl"], cn_strlen($back_link)), $CNCAT["lang"]["field_to_long"]);
                } else {
                    if (!empty($back_link) && $CNCAT["config"]["add_back_link_host"]) {
                        $u1 = @parse_url($link_url);
        ...
        if (!$wizard || ($step == 1 || $step == 3)) {
            ...
            if ($CNCAT["config"]["add_min_title"] == 1 && empty($title)) {
                $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["site_title"], $CNCAT["lang"]["field_empty"]);
            } elseif (cn_strlen($title) < $CNCAT["config"]["add_min_title"]) {
                $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["site_title"], $CNCAT["config"]["add_min_title"]), $CNCAT["lang"]["field_to_short"]);
            } elseif (cn_strlen($title) > $CNCAT["config"]["add_max_title"]) {
                $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["site_title"], $CNCAT["config"]["add_max_title"], cn_strlen($title)), $CNCAT["lang"]["field_to_long"]);
            }
        ...
        if (!$wizard || ($step == 3)) {
            ...
            // check cats exists
            if ($cats && count($cats) < $CNCAT["config"]["add_max_cats"]) {
            ...
            } elseif (count($cats) > $CNCAT["config"]["add_max_cats"]) {
                $errors[] = cn_str_replace("%LEN%", $CNCAT["config"]["add_max_cats"], $CNCAT["lang"]["cats_to_long"]);
            }
        }
        ...
        if (!$wizard || ($step == 3)) {
            ...
            if ($CNCAT["config"]["add_min_email"] == 1 && empty($email)) {
                $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["email"], $CNCAT["lang"]["field_empty"]);
            } elseif (cn_strlen($email) < $CNCAT["config"]["add_min_email"]) {
                $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["email"], $CNCAT["config"]["add_min_email"]), $CNCAT["lang"]["field_to_short"]);
            } elseif (cn_strlen($email) > $CNCAT["config"]["add_max_email"]) {
                $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["email"], $CNCAT["config"]["add_max_email"], cn_strlen($email)), $CNCAT["lang"]["field_to_long"]);
            //} elseif (!@preg_match("/^" . $CNCAT["config"]["add_mail_regexp"] . "$/i" . $CN_STRINGS["preg"], $email)) {
            } elseif ($CNCAT["config"]["add_min_email"] > 0 && !@preg_match("/^[^@\s]+@[^@\s]+\.[a-z0-9]+$/i" . $CN_STRINGS["preg"], $email)) {
        ...
        if (!$wizard || ($step == 3)) {
            ...
            if ($CNCAT["config"]["add_use_author"]) {
                ...
                if ($CNCAT["config"]["add_min_author"] == 1 && empty($author)) {
                    $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["author"], $CNCAT["lang"]["field_empty"]);
                } elseif (cn_strlen($author) < $CNCAT["config"]["add_min_author"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["author"], $CNCAT["config"]["add_min_author"]), $CNCAT["lang"]["field_to_short"]);
                } elseif (cn_strlen($author) > $CNCAT["config"]["add_max_author"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["author"], $CNCAT["config"]["add_amax_author"], cn_strlen($author)), $CNCAT["lang"]["field_to_long"]);
                }
            }
        ...
        if (!$wizard || ($step == 3)) {
            ...
            if ($CNCAT["config"]["add_min_descr"] == 1 && empty($descr)) {
                $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["descr"], $CNCAT["lang"]["field_empty"]);
            } elseif (cn_strlen($descr) < $CNCAT["config"]["add_min_descr"]) {
                $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["descr"], $CNCAT["config"]["add_min_descr"]), $CNCAT["lang"]["field_to_short"]);
            } elseif (cn_strlen($descr) > $CNCAT["config"]["add_max_descr"]) {
                $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["descr"], $CNCAT["config"]["add_max_descr"], cn_strlen($descr)), $CNCAT["lang"]["field_to_long"]);
            }
        ...
        if (!$wizard || ($step == 3)) {
            ...
            if ($CNCAT["config"]["add_use_descr_full"]) {
                ...
                if ($CNCAT["config"]["add_min_descr_full"] == 1 && empty($descr_full)) {
                    $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["descr_full"], $CNCAT["lang"]["field_empty"]);
                } elseif (cn_strlen($descr_full) < $CNCAT["config"]["add_min_descr_full"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["descr_full"], $CNCAT["config"]["add_min_descr_full"]), $CNCAT["lang"]["field_to_short"]);
                } elseif (cn_strlen($descr_full) > $CNCAT["config"]["add_max_descr_full"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["descr_full"], $CNCAT["config"]["add_max_descr_full"], cn_strlen($descr_full)), $CNCAT["lang"]["field_to_long"]);
                }
            }
        ...
        if (!$wizard || ($step == 3)) {
            ...
            if ($CNCAT["config"]["add_use_meta_keywords"]) {
                ...
                if ($CNCAT["config"]["add_min_meta_keywords"] == 1 && empty($meta_keywords)) {
                    $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["meta_keywords"], $CNCAT["lang"]["field_empty"]);
                } elseif (cn_strlen($meta_keywords) < $CNCAT["config"]["add_min_meta_keywords"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["meta_keywords"], $CNCAT["config"]["add_min_meta_keywords"]), $CNCAT["lang"]["field_to_short"]);
                } elseif (cn_strlen($meta_keywords) > $CNCAT["config"]["add_max_meta_keywords"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["meta_keywords"], $CNCAT["config"]["add_max_meta_keywords"], cn_strlen($meta_keywords)), $CNCAT["lang"]["field_to_long"]);
                }
            }
        ...
        if (!$wizard || ($step == 3)) {
            ...
            if ($CNCAT["config"]["add_use_meta_descr"]) {
                ...
                if ($CNCAT["config"]["add_min_meta_descr"] == 1 && empty($meta_descr)) {
                    $errors[] = cn_str_replace("%FIELD%", $CNCAT["lang"]["meta_descr"], $CNCAT["lang"]["field_empty"]);
                } elseif (cn_strlen($meta_descr) < $CNCAT["config"]["add_min_meta_descr"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%"), array($CNCAT["lang"]["meta_descr"], $CNCAT["config"]["add_min_meta_descr"]), $CNCAT["lang"]["field_to_short"]);
                } elseif (cn_strlen($meta_descr) > $CNCAT["config"]["add_max_meta_descr"]) {
                    $errors[] = cn_str_replace(array("%FIELD%", "%LEN%", "%COUNT%"), array($CNCAT["lang"]["meta_descr"], $CNCAT["config"]["add_max_meta_descr"], cn_strlen($meta_descr)), $CNCAT["lang"]["field_to_long"]);
                }
            }
        ...
        if (!$wizard || $step == 3) {
            ...
            foreach ($CNCAT["config"]["extfields"]["items"][0] as $name => $item) {
                ...
                switch ($item["type"]) {
                    ...
                    case 3: $ext_fields["`" . $name . "`"] = "'" . mysql_escape_string(cn_substr($_POST[$name], 0, 255)) . "'"; break;
                    case 4:
        ...
        if (!$wizard || ($step == 1)) {
            if ($CNCAT["config"]["add_use_captcha"]) {
                if (call_user_func($CNCAT["config"]["add_use_captcha"] . "Valid") != "DONE") {
        ...
        if (!$errors && (!$wizard || $step == 1) && ($CNCAT["config"]["add_check_link"] || $CNCAT["config"]["add_accept_to_add"] > 0)) {
            $check_result = cn_checkLink(
                  $link_url, 
                  true, 
                  false, 
                  (bool)$CNCAT["config"]["check_pr"], 
                  (bool)$CNCAT["config"]["check_cy"], 
                  $CNCAT["config"]["check_timeout"], 
            ...
            if (!$check_link_url && $CNCAT["config"]["add_accept_to_add"] > 0) {
        ...
        if (isset($check_result["cy"]) && $CNCAT["config"]["link_add_min_cy"] > 0) {
            if ($check_result["cy"] < $CNCAT["config"]["link_add_min_cy"]) {
                $errors[] = cn_str_replace(array("%MIN_CY%", "%CY%"), array($CNCAT["config"]["link_add_min_cy"], $check_result["cy"]), $CNCAT["lang"]["add_link_accept_cy"]);
            }
        }
        ...
        if (!$errors && (!$wizard || $step == 2) && ($CNCAT["config"]["check_back_link"] || $CNCAT["config"]["add_accept_to_add"] == 2)) {
            if (!$CNCAT["config"]["add_use_back_link"] || empty($back_link)) {
                $back_link = $link_url;
            }
            ...
            if (!$check_back_link && $CNCAT["config"]["add_accept_to_add"] == 2) {
        ...
        // if no errors
        if (!$errors) {  
            if ($CNCAT["config"]["add_use_wysiwyg"]) {
            ...
            if (!$wizard) {
                ...
                if ($CNCAT["config"]["add_auto_approve"] == 1) {
                    $item_status = 1;
                } elseif ($CNCAT["config"]["add_auto_approve"] == 2) {
                    if ($check_link_url) {
                        $item_status = 1;
                    }
                }
                ...
                $step = 4;
                $CNCAT_ENGINE->url->redirect("add", "submit");
            } else {
                // STEP 1, create draft
                if ($step == 1) {
                    if ($item_id) {
                        $query = "UPDATE `" . $dbPrefix . "items` SET
                            ...
                            `item_title`='" . mysql_escape_string($title) . "',
                            `item_title_translite`='" . mysql_escape_string(cn_translitEncode($title)) . "' 
                            WHERE item_id=" . $item_id;
                    } else {
                        $item_insert_type = 1;
                        $query = "INSERT INTO `" . $dbPrefix . "items`
                            (`item_id`, `item_status`, `item_title`, `item_title_translite`, `item_insert_date`, `item_insert_type`, `link_url`, `item_image`, `link_favicon`";
                        $query .= ($CNCAT["config"]["add_check_link"] || $CNCAT["config"]["add_accept_to_add"] > 0) ? ", `link_chk_work_res`, `link_chk_work_date`" : "";

                        if ($CNCAT["config"]["add_check_link"] || $CNCAT["config"]["add_accept_to_add"] > 0) {
                            $query .= $CNCAT["config"]["check_pr"] ? ", `link_rating_pr`, `link_rating_pr_neg`, `link_pr_date`" : "";
                            $query .= $CNCAT["config"]["check_cy"] ? ", `link_rating_cy`, `link_rating_cy_neg`, `link_cy_date`" : "";
                        }
                        ...
                        $query .= ")
                            VALUES(
                                ...
                                '',
                                ''";
                        $query .= ($CNCAT["config"]["add_check_link"] || $CNCAT["config"]["add_accept_to_add"] > 0) ? ", " . (int)$check_link_url . ", NOW()" : "";

                        if ($CNCAT["config"]["add_check_link"] || $CNCAT["config"]["add_accept_to_add"] > 0) {
                            $query .= $CNCAT["config"]["check_pr"] ? ", " . $check_result["pr"] . ", " . -$check_result["pr"] .", NOW()" : "";
                            $query .= $CNCAT["config"]["check_cy"] ? ", " . $check_result["cy"] . ", " . -$check_result["cy"]. ", NOW()" : "";
                        }
                    ...
                    $step = 2;
                    $CNCAT_ENGINE->url->redirect("add", "submit");
                // STEP 2, add back_link to draft
                } elseif ($step == 2) {
                    //if ($CNCAT["config"]["add_accept_to_add"] == 2) {
                        $query = "UPDATE `" . $dbPrefix . "items` SET `link_back_link_url`='" . mysql_escape_string($back_link) . "' WHERE `item_id`=" . $item_id;
                        $CNCAT_ENGINE->db->query($query) or $CNCAT_ENGINE->displayErrorDB(mysql_error());
                    //}
                ...
                // STEP 3, create new item
                    ...
                    $query = "SELECT `link_chk_work_res`, `link_chk_back_res` FROM `" . $dbPrefix . "items` WHERE `item_id`=" . $item_id;
                    $res = $CNCAT_ENGINE->db->query($query) or $CNCAT_ENGINE->displayErrorDB(mysql_error());
                    $row = mysql_fetch_assoc($res);

                    $check_link_url = (bool)$row["link_chk_work_res"];
                    $check_back_link = (bool)$row["link_chk_back_res"];
                    ...
                    } elseif ($CNCAT["config"]["add_auto_approve"] == 3) {
                        if ($check_link_url && $check_back_link) {
                            $item_status = 1;
                        }
                    }
                    ...
                    $query = "UPDATE `" . $dbPrefix . "items` SET
                        ...
                        `item_title`='" . mysql_escape_string($title) . "',
                        `item_title_translite`='" . mysql_escape_string(cn_translitEncode($title)) . "',
                        `item_descr`='" . mysql_escape_string($descr) . "',
                        `item_descr_full`='" . mysql_escape_string($descr_full) . "',
                        `item_meta_keywords`='" . mysql_escape_string($meta_keywords) . "',
                        `item_meta_descr`='" . mysql_escape_string($meta_descr) . "',
                        `item_author_email`='" . mysql_escape_string($email) . "',
                        `item_author_name`='" . mysql_escape_string($author) . "'";
                    
                    foreach ($ext_fields as $name => $value) {
                        $query .= ", " . $name . " = " . $value;
                    }
    
                    $query .= $item_status == 1 ? (", `item_submit_date`=NOW(), `item_submit_type`=" . $item_submit_type)  : "";
                    $query .= " WHERE `item_id`=" . $item_id;
    ...
    $CNCAT["lang"]["field_cats"] = cn_str_replace("%LEN%", $CNCAT["config"]["add_max_cats"], $CNCAT["lang"]["field_cats"]);
    $CNCAT["lang"]["field_author"] = cn_str_replace("%LEN%", $CNCAT["config"]["add_max_author"], $CNCAT["lang"]["field_author"]);
    ...
    if (!$wizard) {
        $result = $CNCAT_ENGINE->tpl->renderTemplate("form");
    } else {
        switch ($step) {
            case 1: $result = $CNCAT_ENGINE->tpl->renderTemplate("step1"); break;
            case 2: $result = $CNCAT_ENGINE->tpl->renderTemplate("step2"); break;
            case 3: $result = $CNCAT_ENGINE->tpl->renderTemplate("step3"); break;
        }
    }
    ...
    if (!$wizard || ($step == 3)) {
        ...
        if ($CNCAT["config"]["add_short_wysiwyg"]) {
            ...
            $result = cn_str_replace("{DISPLAY SHORT_EDITOR}", "<textarea name=\"descr\" cols=\"1\" rows=\"1\">" . htmlspecialchars($descr) . "</textarea>", $result);
        }

        if ($CNCAT["config"]["add_use_wysiwyg"]) {
            $result = cn_str_replace("{DISPLAY EDITOR}", $CNCAT_ENGINE->render->renderTextEditor("descr_full", $descr_full), $result);
        } else {
            $result = cn_str_replace("{DISPLAY EDITOR}", "<textarea name=\"descr_full\" cols=\"1\" rows=\"1\">" . htmlspecialchars($descr_full) . "</textarea>", $result);
        }
    }
